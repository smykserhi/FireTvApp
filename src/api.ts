...
const fetchConfig=(token="", met= "GET" )=> {   
    ...
    let api = {
        ...
        getMyList: (token:string, limit: number = 20, offset: number = 0) => {
            return fetch(`${domain}/api/v2/list??offset=${offset}&limit=${limit}`, fetchConfig(token)).then((res) => res.json())
        },
        addToMyList: (token:string, id:string) => {
            return fetch(`${domain}/api/v2/list/${id}`, fetchConfig(token, "POST")).then((res) => res.json())
        },
        removeFromMyList: (token:string, id:string) => {
            return fetch(`${domain}/api/v2/list/${id}`, fetchConfig(token, "DELETE")).then((res) => res.json())
        }
